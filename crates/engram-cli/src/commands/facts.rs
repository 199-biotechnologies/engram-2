...
pub fn list(
    ...
    let facts = match subject {
        Some(s) => {
            ...
            let diary_filter = if diary == "*" { None } else { Some(diary.as_str()) };
            ctx.store
                .list_facts_by_subject(&s_norm, diary_filter, all)?
        }
        None => {
            let diary_filter = if diary == "*" { None } else { Some(diary.as_str()) };
            ctx.store.list_facts(diary_filter, all, limit)?
        }
    };
    ...
    print_success(
        ...
        |_| {
            ...
            for f in &facts {
                let status = if f.superseded_by.is_some() { " [superseded]" } else { "" };
                println!(
                    "  {} -- {} -> {}  (conf={:.2}){}",
...
pub fn show(ctx: &AppContext, subject: String, diary: String) -> Result<(), CliError> {
    let s_norm = normalize(&subject);
    let diary_filter = if diary == "*" { None } else { Some(diary.as_str()) };
    let facts = ctx
        .store
        .list_facts_by_subject(&s_norm, diary_filter, true)?;
